function [P,z,T,phi,F,dF,F_pd,dF_pd,F_px,dF_px,Pc_cum,Pc,tc,phi_melt,Xun_pd,X_pd,n_pd,r_pd,c_oxwt_pd,C_oxwt_pd,avC_oxwt_pd,Mgno_pd,bulkD_pd,Fo_pd,ctl_pd,Ctl_pd,avCtl_pd,ctol_pd,Ctol_pd,avCtol_pd,KD_pd,Na2O_res]=plumpudding_20_08_14(T0,phi0)

global Tp Cp_pd alpha_pd rho_pd rho_l_pd delS_pd Cp_px alpha_px rho_px ...
    rho_l_px delS_px Mcpx r0 r1
...
Pc_cum=zeros(length(P),1);
Pc_cum_px=zeros(length(P),1);
...
for i=1:(length(P)-1)
    ...
    if T(i)<px_Tsol(P(i+1)) && P(i+1)>Pc_cum(i)
        T(i+1)=fun_adiabat(P(i+1),phi(i));
        F_px(i+1)=F_px(i);
        F_pd(i+1)=F_pd(i);
        
    else if T(i)>px_Tsol(P(i+1)) && T(i)<pd_Tsol(P(i+1)) && P(i+1)>Pc_cum(i)
            ...
            T(i+1)=real(T(i)+(1/6)*(j_1+2*j_2+2*j_3+j_4)*h);
        ...
        else if phi(i)==1 && T(i)>px_Tcpx(P(i+1)) && T(i)<px_Tsol(P(i+1)) && P(i+1)>Pc_cum(i)
                ...
                T(i+1)=real(T(i)+(1/6)*(j_1+2*j_2+2*j_3+j_4)*h);
            ...
            else if T(i)>pd_Tsol(P(i+1)) && T(i)<px_Tcpx(P(i+1)) && X_pd(i,3)>0 && P(i+1)>Pc_cum(i)
                    ...
                    T(i+1)=real(T(i)+(1/6)*(j_1+2*j_2+2*j_3+j_4)*h);
                ...
                else if T(i)>pd_Tsol(P(i+1)) && T(i)<px_Tliq(P(i+1)) && T(i)>px_Tcpx(P(i+1)) && X_pd(i,3)>0 && P(i+1)>Pc_cum(i)
                        ...
                        T(i+1)=real(T(i)+(1/6)*(j_1+2*j_2+2*j_3+j_4)*h);
                    ...
                    else if X_pd(i,3)==0 && T(i)<px_Tcpx(P(i+1)) && P(i+1)>Pc_cum(i)
                            ...
                            T(i+1)=real(T(i)+(1/6)*(j_1+2*j_2+2*j_3+j_4)*h);
                        ...
                        else if T(i)>px_Tcpx(P(i+1)) && T(i)<px_Tliq(P(i+1)) && X_pd(i,3)==0 && T(i)<pd_Tliq(P(i+1)) && P(i+1)>Pc_cum(i) && F(i)>0
                            ...
                            else if T(i)>px_Tliq(P(i+1)) && T(i)<pd_Tliq(P(i+1)) && X_pd(i,3)>0 && P(i+1)>Pc_cum(i)
                                ...
                                else if T(i)>px_Tliq(P(i+1)) && T(i)<pd_Tliq(P(i+1)) && P(i+1)>Pc_cum(i) 
    ...
    F(1)=phi(1)*F_px(1)+(1-phi(1))*F_pd(1);
    F(i+1)=phi(i+1)*F_px(i+1)+(1-phi(i+1))*F_pd(i+1);
    dF(i+1)=F(i+1)-F(i);
    
    if F(i+1)>F(1)
        Pc_cum(i+1)=sum(-h*(phi.*F_px+(ones(length(P),1)-phi).*F_pd)./(ones(length(P),1)-phi.*F_px+(ones(length(P),1)-phi).*F_pd));
        Pc_cum_px(i+1)=sum(-h*phi.*F_px./(ones(length(P),1)-phi.*F_px));
    end
...
Pc=max(nonzeros(Pc_cum));
tc=Pc*10^6/(10*2900);
Pc_px=max(nonzeros(Pc_cum_px));
phi_melt=Pc_px/Pc;
u=length(P)+round(Pc/h)-nnz(dF_pd)+1;

for i=u:length(P)-1
    %MAJOR ELEMENT MELT COMPOSITION
    KD_pd(u,:)=fun_KD(X_pd(u,:));
    Mgno_pd(u)=fun_Mg_no(KD_pd(u,7),bulk_catmol_pd(6),bulk_catmol_pd(5));
    Mgno_mol_pd(u)=fun_Mg_no_mol(KD_pd(u,7),bulk_catmol_pd(6),bulk_catmol_pd(5));
    Na2O_res(u)=bulk_oxwt_pd(10);
    c_oxwt_pd(u,:)=pd_maj_comp(P(u),Mgno_pd(u),X_pd(u,:),n_pd(u,1:6),Na2O_res(u),dF_pd(u));
    c_oxmol_pd(u,:)=fun_oxmol(c_oxwt_pd(u,:));
    c_catmol_pd(u,:)=fun_catmol(c_oxwt_pd(u,:));
    c_catwt_pd(u,:)=fun_catwt(c_oxwt_pd(u,:));
    res_catmol_pd(u,:)=pd_res_catmol(dF_pd(u),bulk_catmol_pd(6),bulk_catmol_pd(5),c_catmol_pd(u,:));
    ...
    c_oxwt_pd(i+1,:)=pd_maj_comp(P(i+1),Mgno_pd(i+1),X_pd(i+1,:),n_pd(i+1,1:6),Na2O_res(i),dF_pd(i+1));
    ...
    Na2O_res(i+1)=(Na2O_res(i)-c_oxwt_pd(i+1,6).*dF_pd(i+1))./(1-dF_pd(i+1));
    ...
    C_catwt_pd(u,:)=fun_catwt(C_oxwt_pd(u,:));
    C_catwt_pd(i+1,:)=fun_catwt(C_oxwt_pd(i+1,:));
    Mgno_mol_ptav_pd(u)=C_oxmol_pd(u,4)/(C_oxmol_pd(u,4)+C_oxmol_pd(u,3));
    Mgno_mol_ptav_pd(i+1)=C_oxmol_pd(i+1,4)/(C_oxmol_pd(i+1,4)+C_oxmol_pd(i+1,3));
...
for s=1:7
    avCun_oxwt_pd(s)=sum(C_F_pd(u:length(P),s))./sum(F_pd(u:length(P)));
end
...
for i=u:(length(P)-1)
    ...
    for j=1:6
        for t=1:8
            ...
            bulkD_pd(u,:)=bulkD_pd(u+1,:);
            
            %Initial trace element composition of unnmelted DMM
            cts_pd(u,:)=[2000,1200,79,2500,798,106,16.3,56];
            
            %Instantaneous melt trace element composition
            ctl_pd(u,t)=fun_ctl(dF_pd(u),cts_pd(u,t),bulkD_pd(u,t),n_pd(u,:),D_pd(u,:,t));
...
for i=u:(length(P)-1)
    for t=1:8
        Ctl_pd(u,t)=ctl_pd(u,t);
        Ctl_pd(i+1,t)=sum(ctl_dF_pd(u:i+1,t))./F_pd(i+1);
    end   
end
...
for i=u:(length(P)-1)
    for t=1:8
        Ctl_F_pd(u,t)=Ctl_pd(u,t).*F_pd(u);
        Ctl_F_pd(i+1,t)=Ctl_pd(i+1,t).*F_pd(i+1);
    end
end

for t=1:8
    avCtl_pd(t)=sum(Ctl_F_pd(u:length(P),t))./sum(F_pd(u:length(P)));
end
...
for i=u:(length(P)-1)
    ...
    Fo_pd(u)=fun_Fo(c_oxmol_pd(u,4),c_oxmol_pd(u,3),KD_pd(u,1));
    ...
    Fo_ptav_pd(u)=fun_Fo(C_oxmol_pd(u,4),C_oxmol_pd(u,3),KD_pd(u,1));
    ...
    avFo_pd=fun_Fo(avC_oxmol_pd(4),avC_oxmol_pd(3),KD_pd(u,1));
    
    %Temperature of shallow basalt liquidus at 6km (0.2GPa) in crust
    %1)for instantaneous melts
    Tbasliq_pd(u)=fun_Tbasliq(0.2,c_oxwt_pd(u,:),Mgno_mol_pd(u));
    Tbasliq_pd(i+1)=fun_Tbasliq(0.2,c_oxwt_pd(i+1,:),Mgno_mol_pd(i+1));
    %2)for point average melts
    Tbasliq_ptav_pd(i+1)=fun_Tbasliq(0.2,C_oxwt_pd(i+1,:),Mgno_mol_ptav_pd(i+1));
    %3)for point and depth average melts
    avTbasliq_pd=fun_Tbasliq(0.2,avC_oxwt_pd(:),avMgno_mol_pd);
    ...
    D_shallow_pd(u,:)=D_shallow_pd(u+1,:);
    %2)for point average melts
    Dol_Mg_ptav_pd(u)=fun_Dol_Mg(C_oxmol_pd(u,4),C_oxmol_pd(u,3));
    ...
    D_ptav_pd(u,:)=D_ptav_pd(u+1,:);
    %3)for point and depth average melts
    avDol_Mg_pd=fun_Dol_Mg(avC_oxmol_pd(4),avC_oxmol_pd(3));
    avD_pd=fun_Dol(avTbasliq_pd,avC_oxmol_pd,avC_oxwt_pd,KD_pd(u,1),avDol_Mg_pd);
    
    %Minor element concentration of primitive olivine
    for j=1:6
        for t=1:8
            %1)incremental fractional melts
            ctol_pd(u,t)=ctl_pd(u,t).*D_shallow_pd(u,t);
            ctol_pd(i+1,t)=ctl_pd(i+1,t).*D_shallow_pd(i+1,t);
            %2)pt average melts
            Ctol_pd(u,t)=Ctl_pd(u,t).*D_ptav_pd(u,t);
...
% F(F==0.000001)=nan;
% F_pd(F_pd==0.000001)=nan;
% F_px(F_px==0.000001)=nan;
% c_oxwt_pd(c_oxwt_pd==0)=nan;
% Fo_pd(Fo_pd==0)=nan;
% Fo_ptav_pd(Fo_ptav_pd==0)=nan;
...
% 
% subplot(2,2,1);
% plot(bulkD_pd(u:length(P),:),P(u:length(P)));
...
% 
% subplot(2,2,3);
% plot(P(u:length(P)),ctl_pd(u:length(P),1),P(u:length(P)),ctl_pd(u:length(P),2));
% title('instantaneous melt comp');
% xlabel('P');
% ylabel('melt comp/ppm');
% legend('Ni','Mn')
% 
% subplot(2,2,4);
