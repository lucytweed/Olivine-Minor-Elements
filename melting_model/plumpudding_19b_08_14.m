function [Tcpx_pd,Tsol_pd,Tlherzliq_pd,P,z,T,phi,F,dF,F_pd,dF_pd,F_px,dF_px,Xun_pd,X_pd,n_pd,r_pd,c_oxwt_pd,C_oxwt_pd,avC_oxwt_pd,Mgno_pd,bulkD_pd,Fo_pd,ctl_pd,Ctl_pd,avCtl_pd,ctol_pd,Ctol_pd,avCtol_pd]=plumpudding_20_08_14(T0,phi0)
...
for i=1:(length(P)-1)
    ...
    Tcpx_pd(1)=pd_Tcpx(P(1));
    Tcpx_pd(i+1)=pd_Tcpx(P(i+1));
    Tsol_pd(i+1)=pd_Tsol(P(i+1));
    Tlherzliq_pd(i+1)=pd_Tlherzliq(P(i+1));
    ...
    if T(i)<px_Tsol(P(i+1))
        ...
        F_pd(i+1)=F_pd(i);
            ...
            T(i+1)=real(T(i)+(1/6)*(j_1+2*j_2+2*j_3+j_4)*h);
                ...
                T(i+1)=real(T(i)+(1/6)*(j_1+2*j_2+2*j_3+j_4)*h);
            ...
            else if T(i)>pd_Tsol(P(i+1)) && T(i)<px_Tcpx(P(i+1)) && X_pd(i,3)>0
                    ...
                    T(i+1)=real(T(i)+(1/6)*(j_1+2*j_2+2*j_3+j_4)*h);
                ...
                else if T(i)>pd_Tsol(P(i+1)) && T(i)<px_Tliq(P(i+1)) && T(i)>px_Tcpx(P(i+1)) && X_pd(i,3)>0
                        ...
                        T(i+1)=real(T(i)+(1/6)*(j_1+2*j_2+2*j_3+j_4)*h);
                    ...
                    else if X_pd(i,3)==0 && T(i)<px_Tcpx(P(i+1))
                        ...
                        else if T(i)>px_Tcpx(P(i+1)) && T(i)<px_Tliq(P(i+1)) && X_pd(i,3)==0 && T(i)<pd_Tliq(P(i+1))
                            ...
                            else if T(i)>px_Tliq(P(i+1)) && T(i)<pd_Tliq(P(i+1)) && X_pd(i,3)>0
...
for i=u:length(P)-1
    ...
    c_oxwt_pd(u,:)=pd_maj_comp(P(u),Mgno_pd(u));
